...
from category_model_builders import (
    build_tfidf_enriched_logreg_parameters,
    build_tfidf_enriched_logreg_pipeline,
...
def build_pipeline() -> Pipeline:
    return build_tfidf_enriched_logreg_pipeline()
...
def main() -> int:
    ...
    output_path = resolve_repo_path(
        args.output or default_output_path(prefix="tfidf-enriched-logreg", dataset_path=dataset_path)
    )
    ...
    pipeline.fit(
        build_model_inputs(train_items, input_format="record"),
    ...
    artifact = build_artifact(
        ...
        run_id=args.run_id
        or default_run_id(prefix="tfidf-enriched-logreg", dataset_path=dataset_path),
        run_date=args.run_date,
        commit_sha=args.commit_sha,
        method="tfidf_enriched_logistic_regression",
        label_order=label_order,
        train_item_count=len(train_items),
        parameters=build_tfidf_enriched_logreg_parameters(),
        ...
        notes=(
            "Sparse category classification baseline trained on the fixed train split of "
            f"{dataset_path.name} using raw text, cleaned item name, and structured slot "
            "features with LogisticRegression."
        ),
    )
